  ...
  <v-expansion-panels popout>
    <v-expansion-panel v-for="subject in subjects" :key="subject.id">
      <v-expansion-panel-header>
        <h4 class="accordionTitle">{{ subject.subject }}</h4>
        <span v-if="subject.total" class="accordionIcon tag is-normal" id="creditsRatio">{{
      ...
      <v-expansion-panel-content>
        <div class="accordionContent">
          <table class="subjectTable">
            ...
            >
              <td class="numCell has-text-left">
                {{ subjectStandards.assessmentNum }}
              </td>
              <td class="nameCell has-text-left">
              ...
              <td class="creditCell has-text-right">
                ...
                <span
                  ...
                  v-bind:class="subjectStandards.completed"
                  class="accordionIcon tag is-normal"
                  >{{ subjectStandards.assessmentCredits }}</span
                >
              </td>
            </tr>
          </table>
        ...
        <div class="accordionFooter">
          <a v-bind:href="getTeacherEmail(subject.teacher)">
            <table class="footerTable">
              <tr>
                <td>
                  Got questions?<br /><span class="has-text-weight-semibold"
                    >Contact {{ subject.teacher }}</span
                  >
                </td>
                <td>
                  <img
                    ...
                    v-bind:src="
                      require('@/assets/staff_photos/' +
                        formatTeacherName(subject.teacher) +
                        '.jpg')
                    "
                  />
                </td>
...
export default {
  ...
  computed: {
    subjects: function() {
      ...
      const subjects = Array.from(
        new Set(this.standards.map((x) => x.subject))
      ).map((subject) => {
        return {
          subject: subject,
  ...
  methods: {
    ...
    getPassed: function(subjectName) {
      ...
      var achieved = achievedStandards
        .map((o) => o.assessmentCredits)
        .reduce((a, c) => a + c, 0);
      var merit = meritStandards
        .map((o) => o.assessmentCredits)
        .reduce((a, c) => a + c, 0);
      var excellence = excellenceStandards
        .map((o) => o.assessmentCredits)
        .reduce((a, c) => a + c, 0);
    ...
    getTeacherEmail(name) {
      switch (name) {
        ...
        case "Whaea Keri":
          return "mailto: k.james@taiwananga.co.nz";
        case "Pa Alec":
          return "mailto: a.smeaton@taiwananga.co.nz";
        default:
          return "";
      }
    },
